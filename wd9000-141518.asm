...
; r3:2
; r5:4
; r7:6	  	    	     		 instruction parameter
; r8:9                       IR          instruction register (current
;                                            word of program)
...
;         g=5        6       BP          mark stack base pointer
...
tt_95:	riw1	ipch,ipcl	; r9:8, tr := [ipc++]; load ics
	...
	mbf	r8,r6
	ll	0x0,r7		; 005: translation 6b (row 11): return
...
tt_94:	mbf	r9,r6
	...
	mb	r8,r6
	rtsr			; reset translation state ; 016: translation 6b (row 00): return (overridden by rtsr instruction)
	

; from end of instruction (translation 3e),
; if no interrupt, and translation state = X0
; fetch word, move low byte to r6, new translation state 100
...
; fetch word indirect from r9:8, move high byte to r6, new translation state 010
tt_06:	r	r9,r8		; r9:8, tr := [r9:8]; load ics
	iw	0x3,r8

; from end of instruction (translation 3e),
; if no interrupt, and translation state = 11
; move high byte to r6, new translation state 010
...
tt_92:	mb	r9,r6
	...
	mb	r8,r7		; 024: translation 6b (row 12): return
...
tt_16:
L031:	dw1	spl,spl		; write opcode to [--sp]
	w	sph,spl,rsvc
	ob	r6,r6
...
tt_20:	dw1	spl,spl,lrr	; 034: translation 1f subr (row 39): get UB parameter
	w	sph,spl,rsvc	; write parameter to [--sp]
	ow	r7,r6
...
tt_17:	ll	0x0,r7		; r7:6 := ((r7:6) & 0xff) + 0xe1
	al	0xe1,r6		;   (to get offset in range 1..16)
				;   result is [MP + offset]
	aw	mpl,r6		; 03f: translation 73 (row 70) - write [r7:6] to stack and end inst


; opcode 0x87: LDL - Load Local Word (from translation 3d)
tt_27:	nop	,lrr		; 040: translation 7a subr (row 35): get B parameter
				; result is [MP + B]
	aw	mpl,r6		; 041: translation 73 (row 72, 73) - write [r7:6] to stack and end inst


; opcode 0x30..0x3f: SLDOi - Short Load Global Word (from translation 3d)
tt_18:	ll	0x0,r7		; r7:6 := ((r7:6) & 0xff) + 0xd1
	al	0xd1,r6		;  (to get offset in range 1..16)
	ll	0x5,r4
	lgl	r4
	aw	gl,r6		; 046: translation 73 (row 71) - write [r7:6] to stack and end inst


; opcode 0x85: LDO - Load Global Word (from translation 3d)
tt_25:	ll	0x5,r4
	lgl	r4,lrr		; 048: translation 7a subr (row 35): get B parameter
	aw	gl,r6		; 049: translation 73 (row 72, 74) - write [r7:6] to stack and end inst
...
tt_29:	mw	mpl,r4,lrr	; 04a: translation 1f subr (row 40): get DB parameter
	jzf	get_lex_level,lrr	; if lex level not zero, find it
	jsr	get_parameter_b
	aw	r4,r6		; 04d: translation 73 (row 74) - write [r7:6] to stack and end inst


; opcode 0x9a: LDE - LoaD word Extended
op_lde:	jsr	get_parameter_ub_sb_db
	jsr	segment_lookup_r76
	jsr	get_parameter_b
	aw	r2,r6		; 051: translation 73 (row 73) - write [r7:6] to stack and end inst


; opcode 0x82: LCA - Load Constant Address (from translation 3d)
tt_22:	ll	0x4,r4		; point to SEGB
	lgl	r4,lrr		; 053: translation 7a subr (row 36): get B parameter
	aw	gl,r6		; result is SEGB + B
...
; after having popped one word from stack into r3:2
; and read but not popped a second word into r7:6, base address
tt_45:	slbf	r3,r8
	srwcf	r3,r3	; convert word index to byte index and carry flag
	aw	r6,r2	; add base address

	r	r3,r2	; read word into r7:6
	iw	0x0,r6

	cmb	r7,r6		; if carry, move high byte to low byte

	w	sph,spl,rsvc	; write result to stack and end inst
	ob	r6,r6


; opcode 0xc8: STB - STore Byte (from translation 37)
; after having popped two words from stack into r3:2, r2 = byte to store
; and r5:4, byte index
tt_65:	riw1	sph,spl	; pop third word from stack into r7:6, base address
	iw	0x0,r6

	slbf	r5,r8
	srwcf	r5,r5	; convert word index to byte index and carry flag
	aw	r6,r4	; add base address

	r	r5,r4	; get original word content into r7:r6
	iw	0x4,r6	;   RMW

	cmb	r2,r7	; if carry, result high byte = byte to store
	cmb	r6,r2,rsvc   ; if carry, result low_byte = orig low byte

			; if no carry, r7 high byte = orig,
			;              r2 low byte = byte to store

	ow	r7,r2	; store the word back, end inst (rsvc on prev uinst)
...
; r3:2 = rightmost bit pos
; r5:4 = field width
tt_66:	mb	r4,r8		; move field width in r8 for later use
	mbf	r2,r4		; move rightmost bit pos into r4

	r	r7,r6		; read word containing bit field into r3:2
	iw	0x0,r2

	jzt	L097		; if rightmost bit pos is zero, no need to align

L094:	srw	r3,r3		; shift bit field right
	db1	r4,r4		; decrement rightmost bit pos
	jzbf	L094

L097:	jsr	compose_bit_mask	; construct right-aligned bit mask
...
; after having popped two words from stack into r3:2 and r5:4
;   r3:2 = value to store into packed field
;   r5:4 = rightmost bit pos
tt_67:	riw1	sph,spl		; pull third word from stack
	ib	0x1,r8		; r8 = packed field width in bits

	jsr	compose_bit_mask	; contruct right-justified mask in r7:6

	mbf	r4,r4		; rightmost bit pos is zero?
	jzt	L0a2		;   yes, no adj. needed to value or mask

L09e:	slw	r2,r2		; shift value left
	slw	r6,r6		; shift mask left
	db1	r4,r4		; decrement rightmost bit pos
	jzbf	L09e

L0a2:	lgl	r4		; r4 is 0 now; use gh:gl as scratch

	riw1	sph,spl		; pull addr of word containing packed field into r5:4
	iw	0x0,r4

	nw	r6,r2

	r	r5,r4		; RMW the word containing the packed field
	iw	0x4,gl

	ncw	r6,gl		; and with complement of mask
	orw	r2,gl,rsvc	; or in new value, end inst

	ow	gh,gl		; write updated value
...
; r7:6 = count
move_words:
	ll	0x0,r8		; ind reg 0 for scratch
	lgl	r8
	jzt	L0bb

L0b5:	riw1	r3,r2		; scratch := [r3:2++]
	iw	0x0,gl

	wiw1	r5,r4		; [r5:4++] := scratch
	ow	gh,gl
...
; r3:2 = old sp = source addr
; r6 = word count
	aw	r6,spl		; adjust sp to pop words

	riw1	sph,spl		; pop dest addr into r5:4
	iw	0x0,r4
...
; r3:2 = source
; r5:4 = dest
tt_62:	jsr	get_parameter_b		; get word count into r7:6
	jmp	move_words
...
get_parameter_ub_sb_db:
	nop			; 0c6: translation 1f (row 46): get UB/SD/DB parameter
...
; find mcsw for a lex level
; enter with current MPL in r5:4, desired lex level in r6
get_lex_level:
	al	0xfd,r4	        ; back up from msseg/msflag field to msstat field
	cdb	r5

L0d5:	r	r5,r4		; lm := lm^.m.msstat
	iw	0x0,r4

	db1	r6,r6		; dec lex level and loop
	jzbf	L0d5

	al	0x3,r4		; increment pointer to msseg/msflag field
	cib	r5		; 0da: translation 6b (row 01): return
...
; after having read but not popped one word from stack into r7:6
...
; after having popped one word from stack into r3:2
; r3:2 = unscaled index
tt_80:	jzt	discard_b_param	; if index = 0, just get param and discard

	mw	r2,r4,lrr	; 0e9: translation 7a subr (row 32): get B parameter
				; r6 := size of element

	jsr	multiply_int	; mulitply to scale the index

	r	sph,spl		; RMW base addr from stack w/o adjust
	iw	0x4,r4

	aw	r4,r2,rsvc	; add scaled index
...
; after having popped one word from stack into r3:2
; and read but not popped a second word into r7:6
...
tt_32:	riw1	sph,spl
	...
	jzt	L164		; if multiplicand is zero, result is zero
...
L113:	r	sph,spl
	...
	tcw	r4,r4
	ocb	r8,r8
L119:	jsr	multiply_int
	srbf	r8,r8

L11b:	jcf	L107
	tcw	r2,r4

; write r5:r4 to stack (without adjustment) as instruction result, and
...
L144:	r	sph,spl		; r3:2 := [sp++]
	...
	jzt	L189		; if dividend is zero, result is zero
...
L14b:	cw	r6,r2
	jc8t	L154
	mw	r2,r4
	xw	r2,r2		; 14e: translation 6b (row 02): return
...
L15a:	slwc	r4,r4
	...
	db1	r8,r8
	jzbf	L15a
	icw1	r6,r6		; 160: translation 6b (row 03): return


; from translation 73, unconditional
; push word at [r7:6] on stack and end instruction
tt_15:	r	r7,r6		; r7:6 := [r7:6]
	iw	0x0,r6
...
; write r7:r6 one word result to stack (without adjustment) and end instruction
...
L1d1:	ll	0xf,r6
	...
	srw	r3,r3
	srw	r3,r3
	srw	r3,r3		; 1d6: translation 6b (row 04): return


; compose bit mask based on count in r8
compose_bit_mask:
	ll	0x0,r7	; r7:6 := 0, carry := 1
	ll	0x1,r6
	srbf	r6,r6

	mb	r8,r8	; bit count is zero?
	jzbt	L1df	;   yes, done

L1dc:	slwc	r6,r6	; shift in a one bit from carry
	db1	r8,r8	; count down
	jzbf	L1dc
...
; after having popped one word from stack into r3:2
tt_79:	ll	0x4,r4		; g points to segb
	lgl	r4,lrr		; 253: translation 7a subr (row 36): get B parameter
	aw	gl,r6		; add B parameter (word index) to SEGB
	ll	0x0,r8
	jmp	L2e9
...
L257:	r	gh,gl
	...
	ll	0xfd,r6		; r7:6 := 0xfffd (-3) -- mcsw size - 1
	ll	0xff,r7

	mw	r6,r8
	lgl	r4		; point to BP
	aw	gl,r6		; r7:6 := lex parent MSCP pointer
	aw	mpl,r8		; r9:8 := caller MSCP pointer
	mw	spl,mpl		; MP := SP

	wiw1	mph,mpl		; write lex parent MCSP pointer
	ow	r7,r6

	wiw1	mph,mpl		; write caller MCSP pointer
	ow	r9,r8

	wiw1	mph,mpl		; write return IPC (byte offset)
	ow	ipch,ipcl

	w	mph,mpl		; write caller segment #
	ob	r5,r5,rsvc	; end instruction
	mw	r2,ipcl		; IPC := r3:2
...
op_cxg:	ll	0x5,r4,lrr	; BP
				; 294: translation 1f subr (row 52): get UB parameter
	jmp	L29c
...
; call external procedure with r5:4 pointing to markstack.msseg/msflag
L298:	lgl	r6
	mw	r4,gl
	ll	0x0,r4
	mw	r2,r6

L29c:	jsr	segment_lookup_r76
	jsr	L2a6
	jsr	get_parameter_ub_sb_db	; get procedure number
L29f:	sw	gl,ipcl
	mw	r2,gl
	jmp	L257
...
L2a6:	r	r7,r6		; increment segment reference count
	iw	0x4,r6
	icw1	r6,r6
	ow	r7,r6

L2aa:	r	gh,gl		; read # of words in segment
	iw	0x0,r6
	aw	gl,r6		; compute end addr of segment

	r	r7,r6		; r5 = segment number? procedure count?
	ib	0x1,r5		; 2ae: translation 6b (row 05): return


; system segment
L2af:	ll	0x2,r2		; SSV/SDP
	lgl	r2
	
	aw	gl,r6		; add segment number to SSV

; system or user segment
L2b2:	ll	0x4,r2		; set G to segb - current segment pointer
	lgl	r2

	r	r7,r6		; r7:6 := sib
	iw	0x0,r6

	riw2	r7,r6		; r3:2 := segment base, r7:6 points to sib->segrefs
	iw	0x0,r2		; 2b7: translation 6b (row 06): return

; subroutine to look up a segment (8-bit seg # in r5)
segment_lookup_r5:
	mbf	r5,r6		; segment number?

; subroutine to look up a segment (8-bit seg # in r6), NB true for user, false for system segment
segment_lookup_r6:
	ll	0x0,r7

; subroutine to look up a segment (16-bit seg # in r7:r6), NB true for user, false for system segment
segment_lookup_r76:
	jnf	L2af		; positive, system segment

; negative, user segment
	nl	0x7f,r6
	...
	al	0xb,r2		; r2 := CTP^.sibsvec
	cib	r3
	r	r3,r2
	iw	0x0,r2
	
	aw	r2,r6		; add segment number to sibsvec
	jmp	L2b2
...
op_rpu:	dw1	mpl,mpl		; sp := mp (adjusted)
	...
	aw	r6,spl		; sp += B parameter
	jzt	L2dc

	jsr	segment_lookup_r5
	jsr	L2aa
	mw	r2,gl
	jsr	segment_lookup_r5
...
; continuation of XJP
L2e9:	riw1	r7,r6		; get low index from case table
	iw	0x0,r4
	swf	r4,r2		; subtract case number
	jnt	op_nop		; out of range, do nothing
	lgl	r8
	riw1	r7,r6		; get high index from table
	iw	0x0,gl
	sw	r4,gl
	cwf	r2,gl
	jct	op_nop		; out of range, do nothing
	aw	r2,r6		; index table using adjusted case number
	r	r7,r6		; get table entry
	iw	0x0,r6
	jmp	L250		; jump using r7:6 as byte offset
...
tt_12:	al	0xff,ipcl
	cdb	ipch		; 2fc: translation 6b (row 13): return
...
tt_13:	; from translation 75, TSR = X0, loads TSR := 2
	nop
	nop			; 2fe: translation 6b (row 13): return


; opcode 0xc1: SBR - SuBtract Real (from translation 2f)
; after having popped two words from stack into r3:2 and r5:4
; and read but not popped a third word into r7:6
tt_58:	al	0x80,r3		; negate subtrahend and fall into add
...
op_tab_9x:
	jmp	op_cpl		; 0x90 CPL  - Call Procedure Local
...
op_tab_b8_bf:
	jmp	op_geqpwr	; 0xb8: GEQPWR - set compare >= (supserset of)
...
op_tab_d8_df:
	jmp	op_ixp		; 0xd8 IXP - IndeX Packed array
...
tt_72:	jsr	L44c
	...
	jct	L449
L446:	mbf	r3,r3
L447:	jnf	write_result_true
	jmp	write_result_false

L449:	mbf	r3,r3
	jnt	write_result_true
	jmp	write_result_false
...
L484:	riw1	r3,r2
	...
	riw1	sph,spl
	iw	0x0,gl
	db1f	r8,r8		; 488: translation 6b (row 07): return
...
op_geqbyt:
	...
	jsr	byte_array_compare
	jzt	write_result_true
	jct	write_result_true
	jmp	write_result_false


byte_array_compare:
	w	sph,spl		; write MP to stack to allow use as temp
	ow	mph,mpl
...
L4b4:	r	sph,spl		; restore MP from stack
	iw	0x0,mpl		; 4b5: translation 6b (row 08): return


; ? remove two set operands from stack, then write false and end instruction
L4b6:	cb	r7,r6
	jc8t	L4ba
	sb	r6,r7
L4b9:	ab	r7,r8
...
; write zero (false) to stack and end instruction
write_result_false:
	ll	0x0,r6
...
; write one (true) to stack and end instruction
write_result_true:
	ll	0x1,r6
...
; after having popped one word from stack into r3:2 (semaphore pointer)
op_signal:
	riw1	r3,r2		; r5:4 := [r3:2++] sem.count
	iwf	0x0,r4

	jzt	L51c

L513:	icw1	r4,r4		; sem.count := sem.count + 1
	dw1	r2,r2		; point r3:r2 back to sem.count
	w	r3,r2
	ow	r5,r4
	...
	lgl	r2
	cl	0xfc,gh
	jzbt	L587		; yes, jump to label 6 in deque pseudocode

L51b:	jmp	op_nop		; no, done


L51c:	r	r3,r2		; r7:6 := [r3:2] sem.waitq
	iw	0x0,r6

	cl	0xfc,r7		; sem.waitq = NIL?
	jzbt	L513		; yes
	...
	ll	0x1,r2		; is CTP nil?
	lgl	r2
	cl	0xfc,gh	
	jzbt	L587		; yes, jump to label 6 of deque pseudocode
	...
	jmp	L587		; jump to label 5 of deque pseudocode


; opcode 0xdf: WAIT - WAIT on semaphore
; after having popped one word from stack into r3:2 (semaphore pointer)
op_wait:
	riw1	r3,r2		; r5:4 := [r3:2++] sem.conut
	iwf	0x0,r4		; sem.count = 0?
	jzt	L53f		; yes

	dw1	r4,r4		; r5:4 -= 1  decr. sem.count

	dw1	r2,r2		; [--r3:2] := r5:4
	w,rsvc	r3,r2		; write count back, next instr
	ow	r5,r4


L53f:	r	r3,r2		; r5:4 := [r3:2] sem.waitq
	iw	0x0,r4
...
L57f:	w	r3,r2
	...
	ll	0x3,r2		; g := rq
	lgl	r2		; 586: translation 6b (row 09): return


; label 6 in deque pseudocode
L587:	ll	0x3,r9		; g := rq
	lgl	r9
...
L5af:	wiw2	r9,r8
	ow	ipch,ipcl	; 5b0: translation 6b (row 10): return
